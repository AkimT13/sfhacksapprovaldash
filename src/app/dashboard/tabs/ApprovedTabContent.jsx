...
export default function ApprovedTabContent() {
  ...
  if (loading)
    return (
      <p className="text-center text-gray-500">Loading approved users...</p>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      ...
      <p className="text-lg text-center mb-4">
        Total Approved: {Object.keys(approvedUsers).length}
      </p>
        ...
        {Object.entries(approvedUsers).length === 0 ? (
          ...
          Object.entries(approvedUsers).map(([id, user]) => (
            <div key={id} className="bg-white shadow-lg p-4 rounded-lg">
              ...
              <p>
                <strong>Email:</strong>{" "}
                {user.data?.fields?.find((f) => f.key === "question_gaaLYM")
                  ?.value || "N/A"}
              </p>
              <button
